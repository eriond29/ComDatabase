...
def showIssues():
    ...
    # Handle deletion of an issue
        ...
        if deleteID:
            mycursor.execute("DELETE FROM Issue_Character WHERE IssueID=%s", (deleteID,))
            connection.commit()

    # Get characterID from query string
    CharacterID = request.args.get('CharacterID')  
    print(f"Received CharacterID: {CharacterID}")  

    if CharacterID is not None:
        # Fetch the comic character's name
        mycursor.execute("""
        ...
        print(f"Issues fetched for CharacterID {CharacterID}: {issues}")  
    ...
    # Debugging prints
    print(f"CharacterID being passed to template: {CharacterID}")  
    print(f"Final CharacterID: {CharacterID}")  
