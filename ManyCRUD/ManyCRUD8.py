...
def showIssues():
    ...
    newIssueID = request.args.get('IssueID')  # 'IssueID' from the HTML template (line 34)
    ...
    CharacterID = request.args.get('CharacterID')  # Make sure this matches the query string parameter name
    print(f"Received CharacterID: {CharacterID}")  # Debugging

    # Fetch character name from the ComicCharacter table
    if CharacterID is not None:
        ...
        # Set the page title for character-specific issues
    ...
    mycursor.close()
    connection.close()
    print(f"CharacterID being passed to template: {CharacterID}")  # Debugging
    ...
    return render_template('Issues.html',
                           ...
                           comicCharacterName=comicCharacterName)  # Pass character name to template
