...
def showIssues():
    ...
    #return render_template('Issues.html', characterId=characterID)
    print(characterID)
    #characterID = request.args.get('characterID')  # Get the character_id from the URL
    if characterID is not None:
        
        # Check if the character is being added to a new issue
        registerIssueID = request.args.get('registerIssueID')
        if registerIssueID is not None:
            
            print(f"Inserting CharacterID: {characterID}, IssueID: {registerIssueID}")

            mycursor.execute("""INSERT into Issue_Character (IssueID, CharacterID) values (%s, %s)
                             """, (registerIssueID, characterID))
            connection.commit()

        else:
            print(f"Invalid IssueID or CharacterID: {registerIssueID}, {characterID}")
        ...
        mycursor.execute("""SELECT Issue.IssueID, Issue.SeriesID, Issue.Year, Issue.Author, Issue.Artist, Issue.IssueNumber FROM Issue
                        JOIN Author ON Issue.Author = Author.PersonID
                        JOIN Artist ON Issue.Artist = Artist.PersonID""")
                        #JOIN ComicCharacter ON ComicCharacter.CharacterID = Issue.CharacterID""")         
